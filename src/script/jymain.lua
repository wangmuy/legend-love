...
function instruct_15()          --game over
    ...
    local r
    if co then
        r = require("menu_async").ShowMenuCoroutine(loadMenu,4,0,x,y,0,0,0,0,CC.DefaultFont,C_ORANGE, C_WHITE)
    else
        r = ShowMenu(loadMenu,4,0,x,y,0,0,0,0,CC.DefaultFont,C_ORANGE, C_WHITE)
    end
...
function instruct_58()           --武道大会比武
    ...
    for i = 0,group-1 do
        ...
        for j = 1,num2 do
            ...
            if co then
                local WarAsync = require("war_async")
                warResult = WarAsync.WarMainCoroutine(warnum + startwar, 0)
            else
                warResult = WarMain(warnum + startwar, 0)
            end
            if warResult == true then     --赢
                instruct_0();
                instruct_13();
...
function instruct_62(id1,startnum1,endnum1,id2,startnum2,endnum2)      --播放时空机动画，结束
      ...
      if co then
          local scheduler = require("coroutine_scheduler").getInstance()
          scheduler:waitForTime(5)
          require("input_async").WaitKeyCoroutine()
      else
          lib.Delay(5000);
          lib.GetKey();
          WaitKey();
...
function instruct_64()                 --小宝卖东西
    ...
    local co = coroutine.running()
    local TalkAsync = co and require("talk_async") or nil
    local MenuAsync = co and require("menu_async") or nil
    ...
    local r
    if co then
        r = MenuAsync.ShowMenuCoroutine(menu,5,0,x1,y1,0,0,1,1,CC.DefaultFont,C_ORANGE, C_WHITE)
    else
        r = ShowMenu(menu,5,0,x1,y1,0,0,1,1,CC.DefaultFont,C_ORANGE, C_WHITE);
    end
