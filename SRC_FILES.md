...
| `event_coroutine.lua` | ❌ 未使用 | 事件脚本协程执行器（旧版），已被 event_executor.lua 替代 |
| `instruct_async.lua` | ❌ 未使用 | 事件指令的协程版本，功能已被 async_globals.lua + event_executor.lua 组合替代 |
...
| `lib_file.lua` | ✅ 使用中 | 文件操作封装，提供跨平台的文件读写接口（open/seek/read/write 等） |
| `luabit.lua` | ✅ 使用中 | LuaBit 位运算库（Lua 5.1 兼容），提供 band/bor/bxor/bnot/brshift/blshift 等位操作 |
| `config.lua` | ✅ 使用中 | 游戏配置文件，定义 CONFIG 表（分辨率、路径、音量、缓存、刷新方式等） |
| `script_loader.lua` | ✅ 使用中 | 脚本加载器，封装 love.filesystem.load 和 loadfile，统一处理 .love 包内外的脚本加载 |
| `perf_log.lua` | ✅ 使用中 | 性能日志模块，提供帧时间和函数执行时间统计，用于性能分析 |
| `convert.lua` | ❌ 未使用 | 数据格式转换工具，将原版 Big5 编码的 R3.grp/R3.idx 数据文件转换为 UTF-8 格式。仅用于一次性数据迁移 |
