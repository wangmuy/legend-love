...
- [ ] 3.1 实现 `ItemAsync.UseThing_Type1Async` 函数
- [ ] 3.2 实现装备目标选择菜单（队友选择）
- [ ] 3.3 实现装备条件检查（CanUseThing）
- [ ] 3.4 实现装备分配逻辑（武器/防具）
- [ ] 3.5 处理装备替换（解除原使用者装备）
- [ ] 3.6 显示装备成功/失败信息
- [ ] 3.7 测试装备物品使用流程
...
- [ ] 4.1 实现 `ItemAsync.UseThing_Type3Async` 函数
- [ ] 4.2 实现药品使用目标选择菜单
- [ ] 4.3 实现生命恢复逻辑
- [ ] 4.4 实现内力恢复逻辑
- [ ] 4.5 实现体力恢复逻辑
- [ ] 4.6 处理无需使用的情况（生命/内力已满）
- [ ] 4.7 显示恢复效果信息
- [ ] 4.8 测试药品使用流程

## 5. 剧情物品异步化

- [ ] 5.1 实现 `ItemAsync.UseThing_Type0Async` 函数
- [ ] 5.2 实现剧情事件触发检查
- [ ] 5.3 调用事件执行系统触发剧情
- [ ] 5.4 处理无对应事件的情况
- [ ] 5.5 测试剧情物品使用
...
- [ ] 6.1 实现 `ItemAsync.UseThing_Type2Async` 函数
- [ ] 6.2 实现修炼目标选择菜单
- [ ] 6.3 实现修炼条件检查（内力性质、内力上限）
- [ ] 6.4 实现武功等级增加逻辑
- [ ] 6.5 显示修炼成功/失败信息
- [ ] 6.6 测试秘籍修炼流程
...
- [ ] 7.1 实现 `ItemAsync.UseThing_Type4Async` 函数
- [ ] 7.2 检查是否在战斗状态
- [ ] 7.3 非战斗状态显示"只能在战斗中使用"
- [ ] 7.4 （可选）实现战斗中的目标选择

## 8. 物品使用入口重构

- [ ] 8.1 实现 `ItemAsync.UseThingAsync` 函数
- [ ] 8.2 根据物品类型分发到对应的使用函数
- [ ] 8.3 处理物品使用失败情况
- [ ] 8.4 添加错误处理和日志
...
- [ ] 9.2 更新 `JyMainAsync.Menu_Thing` 调用 `ItemAsync.UseThingAsync`
