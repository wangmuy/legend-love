...
MMAP 场景名称是中文（如"河洛客棧"），玩家需精确输入才能使用场景名命令。这带来两个问题：
1. 中文名称难拼写、难记忆
2. 玩家不知道可去哪些场景
...
**Goals:**
- `list` 改为菜单模式（替代纯文字列表），选择场景前往
- 模式可复用，后续 heal/give/take/wugong 等命令套用
- MMAP 中 `go` 已移除，场景导航统一走 `list` + `choose N`
...
### ADR-002: "菜单模式"放在命令处理器中

- **Context**: `list` 命令的行为从纯文字列表改为菜单模式
- **Decision**: 在 `mmap_smap_handlers.lua` 中调用 `CommandEngine.showMenu(items, title, callback)` 弹菜单
- **Alternatives Considered**:
  - CommandEngine 层统一处理"无参弹菜单" → 需要声明式配置，过度设计
- **Consequences**: 每个命令处理器可自行决定是否弹菜单
...
1. `list` 弹出场景菜单，`choose N` 正确传送到对应场景
2. ESC 关闭菜单回到原状态（不做任何操作）
3. 菜单命令只在 GAME_MMAP 下可用，GAME_START 下不可见
4. SMAP 中 `go <编号>` 保持原行为（非菜单模式）