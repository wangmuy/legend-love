...
- [ ] 1.2 解锁丐帮天龙八部事件（oldevent_523/524/525）
...
- [ ] 1.4 解锁沙漠废墟白马啸西风事件（oldevent_654）
...
- [ ] 1.5 解锁神龙教鹿鼎记事件（oldevent_607/608/609）
...
- [ ] 1.6 解锁黑木崖笑傲江湖事件（oldevent_316/318/323/324/325/326）
...
- [ ] 1.7 解锁回族部落书剑恩仇录事件（oldevent_620/624）
...
- [ ] 1.8 解锁侠客岛侠客行事件（oldevent_352/353）
...
- [ ] 2.1 天宁寺连城诀测试（P7 天宁寺步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p7.spec.js"]`
  DoD:
    - [ ] 在 P7 天宁寺步骤中，天宁寺 NPC 对话 → 触发连城诀事件
    - [ ] 验证终端输出包含连城诀相关文本
    - [ ] 无 gameLoop error

- [ ] 2.2 丐帮天龙八部测试（P7 丐帮步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p7.spec.js"]`
  DoD:
    - [ ] 在 P7 丐帮步骤中，选择 NPC 对话 → 触发天龙八部事件
    - [ ] 验证终端输出包含天龙八部相关文本
    - [ ] 无 gameLoop error

- [ ] 2.3 桃花岛射雕英雄传测试（P7 桃花岛步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p7.spec.js"]`
  DoD:
    - [ ] 在 P7 桃花岛步骤中，黄蓉对话 → 触发射雕英雄传事件
    - [ ] 验证终端输出包含射雕英雄传相关文本
    - [ ] 无 gameLoop error

- [ ] 2.4 沙漠废墟白马啸西风测试（P6 沙漠废墟步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p6.spec.js"]`
  DoD:
    - [ ] 在 P6 沙漠废墟步骤中，NPC 对话 → 触发白马啸西风事件
    - [ ] 验证终端输出包含白马啸西风相关文本
    - [ ] 无 gameLoop error

- [ ] 2.5 神龙教鹿鼎记测试（P6 神龙教步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p6.spec.js"]`
  DoD:
    - [ ] 在 P6 神龙教步骤中，洪教主对话 → 触发鹿鼎记事件
    - [ ] 验证终端输出包含鹿鼎记相关文本
    - [ ] 无 gameLoop error

- [ ] 2.6 黑木崖笑傲江湖测试（P7 黑木崖步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p7.spec.js"]`
  DoD:
    - [ ] 在 P7 黑木崖步骤中，NPC 对话 → 触发笑傲江湖事件
    - [ ] 验证终端输出包含笑傲江湖相关文本
    - [ ] 无 gameLoop error

- [ ] 2.7 回族部落书剑恩仇录测试（P2 回族部落步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p2.spec.js"]`
  DoD:
    - [ ] 在 P2 回族部落步骤中，NPC 对话 → 触发书剑恩仇录事件
    - [ ] 验证终端输出包含书剑恩仇录相关文本
    - [ ] 无 gameLoop error

- [ ] 2.8 古墓九阴真经/神雕侠侣测试（P3 古墓步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p3.spec.js"]`
  DoD:
    - [ ] 在 P3 古墓步骤中，除了小龙女加入外，验证古墓 oldevent_442（九阴真经）可触发
    - [ ] 验证终端输出包含九阴真经相关文本
    - [ ] 无 gameLoop error

- [ ] 2.9 侠客岛侠客行测试（P6 侠客岛步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p6.spec.js"]`
  DoD:
    - [ ] 在 P6 侠客岛步骤中，龙岛主对话 → 触发侠客行事件
    - [ ] 验证终端输出包含侠客行相关文本
    - [ ] 无 gameLoop error

- [ ] 2.10 光明顶倚天屠龙记测试（P5 光明顶步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p5.spec.js"]`
  DoD:
    - [ ] 在 P5 光明顶步骤中，验证 tile event extra=82 可触发
    - [ ] 验证终端输出包含倚天屠龙记相关文本
    - [ ] 无 gameLoop error

- [ ] 2.11 渤泥岛碧血剑测试（P6 渤泥岛步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p6.spec.js"]`
  DoD:
    - [ ] 在 P6 渤泥岛步骤中，袁承志对话 → 触发碧血剑事件
    - [ ] 验证终端输出包含碧血剑相关文本
    - [ ] 无 gameLoop error

- [ ] 2.12 苗人凤居飞狐外传测试（P4 苗人凤居步骤）
  Blast Radius: `["game/engine-web/tests/walkthrough-p4.spec.js"]`
  DoD:
    - [ ] 在 P4 苗人凤居步骤中，tile event extra=30（退敌战斗）已验证
    - [ ] 验证终端输出包含苗人凤相关文本
    - [ ] 无 gameLoop error
...
- [ ] 3.1 圣堂14个书架事件可操作
  ...
  DoD:
    - [ ] 在 P10 圣堂步骤中，look 查看显示 14 个"放置天书"事件
    - [ ] 每个书架事件可 choose 触发，无 gameLoop error
    - [ ] 验证终端输出包含"放置天书"或"是否使用物品"文本

## 4. 全量回归测试
Traceability: [REQ-WT-003]

- [ ] 4.1 运行全部 P1-P10 + save-state 测试
